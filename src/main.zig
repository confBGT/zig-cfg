...
pub fn main() !void {
    ...
    var cfg = try CFG.fromString(std.heap.page_allocator, contents);
    defer cfg.deinit();

    try cfg.convertToChomskyNormalForm();

    const shortest_derivation = try CYK.parse(std.heap.page_allocator, &cfg, &sentence);
    std.debug.print("{any}\n", .{shortest_derivation});
    // try std.testing.expect(shortest_derivation != null);
}
...
test "Short grammar" {
    ...
    const shortest_derivation = try CYK.parse(std.testing.allocator, &cfg, &sentence);
    try std.testing.expect(shortest_derivation != null);
}
